...
<u>对内置(C-like)类型值传递比引用传递高效</u>，但当<u>使用Object-Oriented C++时</u>，由于用户自定义构造函数和析构函数的存在，<u>pass-by-reference-to-const往往更好</u>，尤其使用Template C++时。但对STL而言，迭代器和函数都是在C指针上塑造出来的，因此<u>对STL的迭代器和函数对象而言，值传递反而适用</u>。
...
1. 定义常量指针(constant pointers)：<u>常量定义式常放在头文件，故需要将指针声明为const，如要在头文件内定义一个常量的char*-based字符串，需写const两次</u>
...
2. class专属常量：<u>为将常量的作用域(scope)限制于class内，必须让它成为class的一个成员(member)，为了确保该常量最多只有一份实体，需要让它成为一个static成员。</u>
...
万一该<u>旧编译器不允许static整型class常量完成in class的初值设定，可改用"the enum hack"</u>。原理是：一个属于枚举类型(enumerated type)的数值可当作int被使用。
...
STL迭代器是用指针塑造的，所以迭代器就像个T*指针。声明迭代器为const就像声明一个T\* const指针一样。说明这个迭代器不能指向不同的东西，但所指物的值可改动。<u>若希望迭代器所指物的值不可被改动(即要STL模拟一个const T\* 指针)，应该用const_iterator：</u>
...
若a和b是内置类型，这样必然不合法，为了使我们的自定义类型与内置类型兼容，预防错误的赋值，<u>应将operator*的回传值声明为const</u>。
...
解决办法：<u>利用C++中的mutable，释放掉non-static成员变量的bitwise constness约束</u>：
...
一般情况下，转型(casting)是应当避免的。但在本例const operator[] 做完了non-const版本该做的一切，唯一不同仅在于返回类型多了个const修饰的情况下，将返回值的const转除是安全的。因为non-const operator[]被调用时一定有个non-const对象，否则无法调用non-const函数，所以<u>令non-const operator[] 调用const版本</u>是一个避免代码重复的安全做法。
...
反向做法，即令const啊不能本调用non-const版本是错误的，因为const成员函数不改变其对象的逻辑状态，non-const成员函数却可以。如果在const函数内调用non-const函数，就可能带来对象被改动的风险。
...
最佳处理办法是：<u>永远在使用对象之前先将它初始化。对无任何成员的内置类型，应手工完成</u>。如：
...
应当遵循一个规则：<u>在初值列中列出所有成员变量，以免忘记某些内置类型的成员变量必须要给初值。</u>

当成员变量是const或reference，那就一定要初始化而不是被赋值。

总之，最简单的做法就是：<u>都使用member initialization list</u>。
...
C++有十分固定的成员初始化次序，基类先于派生类初始化，class的成员变量以声明顺序被初始化。因此，为避免混淆，<u>成员初值列中各个成员应保持声明顺序</u>。
...
正确做法是：<u>将每个non-local static对象搬到自己的专属函数内(该对象在此函数内声明为static)。这些函数返回一个reference指向它所含的对象，然后用户调用这些函数，而不直接调用所含对象。</u>
